...
![common 을 중심으로](docs/focus-common.svg)
...
| | 묻는 것 | 못 넘으면 무엇이 생기나 |
|---|---|---|
| **①** | 거의 바뀌지 않는가 | 바뀔 때마다 **재배포 + 전 서비스 버전업** |
...
> **기준 ②의 문턱이 실제로 어디쯤인지가 이 표에서 드러납니다.**
> S3 presigned 는 3개, `AfterCommitExecutor` 는 4개였고 **둘 다 안 넣었습니다.**

---

**「서비스 코드를 안 본다」는 것만으로는 기준이 못 됩니다.**

`RestClientAuthInterceptor` 가 통과한 진짜 근거는 **짝이 여기 있다는 것**입니다.

| | 하는 일 | 어디 있나 |
|---|---|---|
| `HeaderAuthenticationFilter` | 요청 헤더를 읽어 `SecurityContext` 를 채움 | 이 모듈 |
| `RestClientAuthInterceptor` | `SecurityContext` 를 꺼내 요청 헤더로 실음 | **이 모듈이어야 짝이 맞음** |

**채우는 쪽이 이 모듈이므로 꺼내 쓰는 쪽도 이 모듈이어야 합니다.**
`AfterCommitExecutor` 도 서비스 코드를 전혀 보지 않지만 **짝이 없습니다.**
스프링이 주는 API 를 30줄로 감싼 독립 유틸리티이고 이 모듈의 무엇과도 맞물리지
않습니다.
...
```
1곳  private 메서드   ──▶   2곳  별도 클래스   ──▶   3곳  이 모듈
```

`auth` 의 `AfterCommitExecutor` 가 이 경로를 밟았습니다. 원래 `AuthService` 안의
private 메서드였는데 비밀번호 재설정도 같은 것이 필요해져 **2곳이 되자 클래스로
꺼냈습니다.**
...
**이 모듈의 기준은 "거의 바뀌지 않는 것" 입니다.** [7-4](#7-4-무엇을-여기-넣고-무엇을-안-넣나) 의 기준 ① 에 걸린 사례입니다.
...
### 7-9. `X-Original-Trace-Id` 를 손으로 안 넣음
